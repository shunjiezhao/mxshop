...
func InitConfig() {
	...
	configFileName := "shop_cart-web/config/config-pro.yaml"
	if debug {
		configFileName = "shop_cart-web/config/config-debug.yaml"
	}
	err := readFile(configFileName, "yaml")
	if err != nil {
		panic(err)
	}
	readSection("CartWeb", global.ServerConfig)
