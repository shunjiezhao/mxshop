...
import (
	...
	"server/shopcart_service/proto"
)

type shopcartService struct {
	logger *zap.Logger
	db     *gorm.DB
	proto.UnimplementedshopcartServer
}
...
func NewService(config *shopcartSrvConfig) proto.shopcartServer {
...
func (i *shopcartService) SetInv(ctx context.Context, req *proto.GoodsInvInfo) (*emptypb.Empty, error) {
	// 没有新增
	var inv model.shopcart
	...
	if err := i.db.Save(&inv).Error; err != nil {
		i.logger.Error("can not save shopcart", zap.Error(err))
		return nil, status.Error(codes.Internal, "")
	}
	return &emptypb.Empty{}, nil
}

func (i *shopcartService) InvDetail(ctx context.Context, req *proto.GoodsInvInfo) (*proto.GoodsInvInfo, error) {
	var inv model.shopcart
...
func (i *shopcartService) Sell(ctx context.Context, req *proto.SellInfo) (*emptypb.Empty, error) {
	...
	for _, goodInfo := range req.GoodsInfo {
		...
		if result := tx.Where(&model.shopcart{Goods: goodInfo.GoodsId}).First(&inv); result.RowsAffected == 0 {
			tx.Rollback() //回滚之前的操作
...
func (i *shopcartService) Reback(ctx context.Context, req *proto.SellInfo) (*emptypb.Empty, error) {
	// 1. 订单超时归还
	// 2. 订单创建失败，归还之前扣减的
	// 3. 手动归还
	// 扣减库存
