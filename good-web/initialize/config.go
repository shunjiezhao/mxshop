...
func InitConfig() {
	...
	configFileName := "good-web/config/config-pro.yaml"
	if debug {
		configFileName = "good-web/config/config-debug.yaml"
	}
	err := readFile(configFileName, "yaml")
	if err != nil {
		panic(err)
	}
	readSection("GoodsWeb", global.ServerConfig)
